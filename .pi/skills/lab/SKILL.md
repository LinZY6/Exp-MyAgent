...
5. Then follow `.pi/agents/ROSTER.md`: `campaign_gate.must` names the next tool (`call_designer` / `call_experimenter` / `call_reviewer` / `ask_user`). Do not ask which experiment to try. The ledger is `<lab>/fn_fit/experiments.jsonl` unless this lab's CHARTER names another collection.

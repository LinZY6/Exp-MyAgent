...
5. Then **run the campaign yourself**: search → create → run → complete → next idea. Do not ask which experiment to try. The ledger is `<lab>/fn_fit/experiments.jsonl`.

## Bounds (every edit)

Before `read`/`edit`/`write` of project code:

1. `assert_lab_path` on that file.
2. If `in_lab` is false → stop. Do not edit repo `tools/`, `.pi/`, `CHARTER.md`, or anything outside the lab.

Allowed edits: `<lab>/src/fnfit/fit.py`, `world.py`, `linalg.py`, `protocol.json`, this lab's `CHARTER.md`. Do not edit `<lab>/src/fnfit/tools.py` unless the user asks to change the tool API.
